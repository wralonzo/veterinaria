...
import { ModalUpdateExamenComponent } from './modal-update/modal-update.component';
import { Router } from '@angular/router';
import { ModalExamenComponent } from './modal/modal.component';
...
export class ExamenComponent {
  ...
  dataSource = new MatTableDataSource<IExamen>(this.elementData);
  ...
  openModal() {
    const dialogRef = this.dialog.open(ModalExamenComponent);
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.elementData.splice(0, 0, result);
        this.dataSource = new MatTableDataSource<IExamen>(this.elementData);
        this.dataSource.paginator = this.paginator;
  ...
  openModalEdit(idClient: string) {
    ...
    const dialogRef = this.dialog.open(ModalUpdateExamenComponent, {
      width: '400px',
      data: findClient,
    });
    dialogRef.afterClosed().subscribe((result) => {
  ...
  getData() {
    try {
      return this.axiosService.getApi('examen').subscribe(
        (response) => {
